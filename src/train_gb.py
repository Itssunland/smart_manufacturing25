...
Train a Gradient Boosting (HistGradientBoostingClassifier) on your collected drill-session data.

Usage:
  cd <project_root>
  source venv/bin/activate
  python src/train_GB.py
"""
...
OUT_MODEL   = os.path.join(MODELS_DIR, 'gb_material_clf.pkl')
...
def load_data(db_path):
    ...
    if not rows:
        raise RuntimeError(f"No data found in {db_path}. Run data generation first.")
...
def main():
    ...
    # Pipeline: robust scaling + HistGradientBoosting
    pipe = Pipeline([
        ('scaler', RobustScaler()),
